...
def load_glove_embeddings_gdrive(model_type):
    ...
    print('word_index_temp', word_index_temp)
...
def cosine_similarity(x, y):
    ...
    cos_sim = dot_product / (magnitude_x * magnitude_y)
    return np.e ** cos_sim
    ##################################
...
def averaged_glove_embeddings_gdrive(sentence, word_index_dict, embeddings, model_type="50d"):
    ...
    embeddings_list = []
    for word in words:
        embeddings_list.append(get_glove_embeddings(word, word_index_dict, embeddings, model_type))
...
def get_sorted_cosine_similarity(embeddings_metadata):
    ...
    if embeddings_metadata["embedding_model"] == "glove":
        ...
        ##########################################
        x = averaged_glove_embeddings_gdrive(sentence, word_index_dict, embeddings, model_type)
        for idx, category in enumerate(categories):
            y = get_glove_embeddings(category, word_index_dict, embeddings, model_type)
            cosine_sim[idx] = cosine_similarity(x, y)
        ...
        get_category_embeddings(embeddings_metadata)
        x = get_openai_embeddings(sentence, model_name)
        for idx, category in enumerate(categories):
            y = st.session_state[cache_key][category]
            cosine_sim[idx] = cosine_similarity(x, y)
        ...
        get_category_embeddings(embeddings_metadata)
        x = get_sentence_transformer_embeddings(sentence, model_name)
        for idx, category in enumerate(categories):
            y = st.session_state[cache_key][category]
            cosine_sim[idx] = cosine_similarity(x, y)
...
if __name__ == "__main__":
    ...
    if st.session_state.text_search:
        ...
        st.write("")
        st.write(
            "Demo developed by [Lizzy](https://www.linkedin.com/in/lizzyaustad)"
        )
